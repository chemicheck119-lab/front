...
export function SessionExpiredBanner({ authLoginUrl, hasIncident, requestId }: SessionExpiredBannerProps) {
  ...
  const publicPilotSelectionUrl = safeAuthLoginUrl && publicPilotAccess
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {publicPilotSelectionUrl ? (
              <a href={publicPilotSelectionUrl} target="_blank" rel="noreferrer" className="inline-flex min-h-9 items-center gap-1.5 rounded-lg bg-foreground px-3 text-[13px] font-bold text-background">소방서 다시 선택<ExternalLink size={12} /></a>
            ) : safeAuthLoginUrl ? (
            ...
            {requestId && <button type="button" onClick={() => void navigator.clipboard.writeText(requestId).then(() => setCopied(true)).catch(() => setCopied(false))} className="min-h-9 rounded-lg border border-border px-3 text-xs font-semibold">{copied ? "요청 ID 복사됨" : `요청 ID ${requestId} 복사`}</button>}
          </div>
        </div>
      </div>
    </aside>
