...
function ToolButton({ icon, label, detail, badge, emphasized = false, onClick }: {
  ...
  return (
    <button onClick={(event) => onClick(event.currentTarget)} className={`group flex min-h-[72px] w-full items-center gap-3 rounded-xl border px-3 py-2.5 text-left transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sidebar-ring ${emphasized ? "border-primary/55 bg-primary/5 shadow-sm hover:bg-primary/10" : "border-transparent hover:border-sidebar-border hover:bg-sidebar-accent"}`}>
      ...
      {badge && <span className="rounded-full bg-primary/10 px-2 py-0.5 text-xs font-bold text-primary">{badge}</span>}
    </button>
  );
}
    ...
    <>
      <aside className="flex w-[196px] shrink-0 flex-col border-r border-sidebar-border bg-sidebar p-4" aria-label="현장 도구">
        ...
        <div className="mt-auto space-y-2.5 rounded-xl border border-sidebar-border bg-sidebar-accent/55 p-3 text-xs font-medium text-muted-foreground" aria-label="운영 상태">
          ...
          <div className="flex items-center gap-2"><FileClock size={14} className="shrink-0" /><span>{unsavedCount ? `미저장 ${unsavedCount}건` : "미저장 없음"}</span></div>
              ...
              {officialItems.map((item) => (
                <article key={item.key} className="rounded-xl border border-border p-3">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className={`rounded-full px-2 py-1 text-xs font-bold ${item.confirmed ? "bg-emerald-500/10 text-emerald-700 dark:text-emerald-300" : "bg-amber-500/10 text-amber-700 dark:text-amber-300"}`}>{item.confirmed ? "현장 확인됨" : "후보·확인 필요"}</span>
                  </div>
                  <div className="mt-3 grid grid-cols-3 gap-1.5">
                    ...
                    <a href={KOSHA_MSDS_URL} target="_blank" rel="noreferrer" className="flex min-h-11 items-center justify-center gap-1 rounded-lg border border-border text-xs font-semibold hover:bg-muted">KOSHA<ExternalLink size={11} /></a>
      ...
      {activeDialog === "record" && (
        <ToolDialogShell title="현재 사고 기록" description="현재 화면의 대화·분석·현장 확인 상태입니다. 저장 성공 전까지 미저장 기록으로 유지됩니다." onClose={closeDialog}>
          ...
          <div className="mt-4 flex items-center justify-between rounded-xl border border-border bg-secondary/45 p-3"><div><p className="text-xs text-muted-foreground">저장 상태</p><p className="mt-1 text-xs font-bold">{unsavedCount ? `미저장 ${unsavedCount}건` : "미저장 기록 없음"}</p></div><FileClock size={18} className="text-muted-foreground" /></div>
          {!recordAvailable && <p className="mt-3 rounded-xl border border-blue-500/20 bg-blue-500/5 p-3 text-xs leading-relaxed text-blue-700 dark:text-blue-300">현재 대화·분석·확인 내역은 계속 볼 수 있습니다. 서버 기록저장 API가 배포되면 저장 기능을 활성화합니다.</p>}
