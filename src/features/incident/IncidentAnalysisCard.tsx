...
function EvidenceLinks({ evidence }: { evidence: EvidenceCard[] }) {
  if (evidence.length === 0) {
    return <p className="mt-2 rounded-lg bg-accent/10 p-2 text-xs leading-relaxed text-accent">연결된 공식 근거가 없습니다. 현장 제품의 원문 MSDS를 직접 확인해주세요.</p>;
  }
...
function ConfirmationButton({ role, casNumber, displayName, confirmingRole, onConfirm, label }: {
  ...
  return (
    <button
      ...
      className="min-h-11 shrink-0 rounded-lg bg-primary px-3 text-[13px] font-bold text-white shadow-sm disabled:opacity-50"
        ...
        {steps.map((step, index) => (
          <li key={step.id} className={`min-w-0 rounded-lg border px-2 py-2 ${confirmationStepStyle[step.status]}`}>
            <div className="flex items-center gap-1.5">
              {step.status === "COMPLETED" ? <CheckCircle2 size={12} className="shrink-0" /> : step.status === "LOCKED" ? <LockKeyhole size={11} className="shrink-0" /> : <span className="grid h-3 w-3 shrink-0 place-items-center rounded-full bg-current text-[11px] font-bold text-white dark:text-slate-950">{index + 1}</span>}
              <span className="truncate text-xs font-bold">{step.label}</span>
            </div>
            <p className="mt-1 truncate text-xs opacity-80">{step.detail}</p>
          </li>
        ))}
      </ol>
    </section>
...
export function IncidentAnalysisCard({ analysis, onConfirm, confirmingRole }: IncidentAnalysisCardProps) {
  if (!analysis) {
    return (
      <div className="rounded-xl border border-dashed border-border bg-secondary/40 p-4 text-center">
        ...
        <p className="mt-1 text-[13px] text-muted-foreground">신고 내용을 입력하면 물질 후보와 필요한 확인 행동을 정리합니다.</p>
      </div>
    );
  }
  ...
  const confirmedCount = Number(analysis.confirmationGate.incidentConfirmed) + Number(analysis.confirmationGate.facilityConfirmed);
  const incidentCandidate = analysis.substanceCandidates.find((item) => item.role === "INCIDENT");
  const facilityCandidate = analysis.substanceCandidates.find((item) => item.role === "FACILITY");
      ...
      <article className="overflow-hidden rounded-xl border border-border bg-secondary/45">
        ...
        <section className="grid grid-cols-3 gap-1.5 border-b border-border bg-card p-3" aria-label="현장 핵심 정보">
          ...
          <div className={`rounded-lg border p-2.5 ${completed ? completed.riskLevel === "HIGH" ? "border-primary/40 bg-primary/10" : "border-emerald-500/30 bg-emerald-500/10" : "border-border bg-muted/55"}`}>
            ...
            <p className={`mt-1 truncate text-[13px] font-bold ${completed?.riskLevel === "HIGH" ? "text-primary" : ""}`}>{completed ? completed.riskLevelKo : "검토 잠김"}</p>
            <p className="mt-0.5 truncate text-xs text-muted-foreground">{completed ? "CAMEO 공개 규칙" : "두 CAS 확인 후"}</p>
            <p className="mt-1 text-xs font-bold">{completed ? "결과 확인" : "임의 판단 금지"}</p>
          </div>
        </section>
        ...
        <section className="space-y-2 p-3" aria-label="신고 물질 후보">
          {analysis.substanceCandidates.length === 0 && <p className="rounded-lg bg-card p-3 text-[13px] text-muted-foreground">식별 가능한 물질 후보가 없습니다. 라벨·MSDS·운송 문서를 확인해주세요.</p>}
        ...
        {analysis.facilityHistory.status !== "NOT_QUERIED" && (
          <section className="border-t border-border bg-card p-3" aria-label="시설 과거 이력 후보">
            <p className="flex items-center gap-1.5 text-[13px] font-bold"><History size={13} />{analysis.facilityHistory.label}</p>
            <p className="mt-1 rounded-lg bg-accent/10 p-2 text-xs leading-relaxed text-accent">{analysis.facilityHistory.warning || "과거 공개 이력 기반 후보 · 현재 재고 확인 필요"}</p>
                ...
                {analysis.facilityHistory.candidates.map((candidate) => {
                  ...
                  return (
                    <div key={`${candidate.facilityName}-${candidate.casNumber}`} className="flex items-center justify-between gap-2 rounded-lg border border-border bg-secondary/40 p-2.5">
                      <div><p className="text-xs font-semibold">{displayName}</p><p className="mt-0.5 font-mono text-xs text-muted-foreground">CAS {candidate.casNumber} · {candidate.facilityName}</p>{candidate.sourceUrl && <a href={candidate.sourceUrl} target="_blank" rel="noreferrer" className="mt-1 inline-flex items-center gap-1 text-xs font-semibold text-blue-600">공개 이력 원문<ExternalLink size={9} /></a>}</div>
            ...
            ) : <p className="mt-2 text-xs text-muted-foreground">현재 연결된 과거 공개 이력 후보가 없습니다.</p>}
          </section>
        )}

        {completed ? (
          <section className="border-t border-border bg-card p-3" aria-label="충돌 검토 결과">
            <div className="flex items-end justify-between gap-2"><div><p className="text-xs text-muted-foreground">공개 CAMEO 규칙 기준 서수 등급</p><p className={`mt-1 text-lg font-bold ${completed.riskLevel === "HIGH" ? "text-primary" : completed.riskLevel === "MEDIUM" ? "text-accent" : "text-foreground"}`}>{completed.riskLevelKo}</p></div><span className="text-xs text-muted-foreground">확률·백분율 아님</span></div>
            <p className="mt-2 text-[13px] leading-relaxed">{completed.briefText}</p>
        ...
        <footer className="border-t border-border bg-muted/40 text-xs text-muted-foreground">
