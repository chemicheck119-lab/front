...
현재 공개 데모가 실시간으로 받는 것은 사용자가 입력한 신고문 또는 개인정보 없는 합성 SSE다.
지도에 표시하는 출동 위치·경로·ETA는 QA용 합성 데이터이며 실제 브라우저 GPS·길찾기가 아니다. 소방청
...
|---|---|---|---|---|
| Contest Live | BE SSE로 수신한 개인정보 없는 공개 합성 지령 | 배포된 BFF·AI + 로컬 QA JSON | `서버 연동` + `공개 합성 신고` | 발표 주 시연 |
| Manual Live | 상황실·대원이 직접 입력 | 배포된 BFF·AI | `서버 연동` | 기관 pilot 전 운영 검증 |
| Dispatch Pilot | 승인된 지령 adapter | 동일 BFF·AI·DB | 실제 입력 출처 | 기관 협약 후 |
| Offline Demo | 저장소 fixture | 브라우저 메모리 | `오프라인 시연` | 인터넷 장애 백업 |
