...
`/` 메인페이지는 **시범 전화 → 전사 검토 → 분석·확인 → 대응 기록**의 흐름을 안내합니다.
기존 좌우 히어로·등장 애니메이션을 유지하며, 오른쪽의 작은 전화 CTA 아래에 **상황실 모니터링·소방대원용 지역·소방서 선택**을 주 진입으로 표시합니다.
전화는 밝은 보조 카드, 상황실은 전체 너비의 대응 화면 버튼으로 구분합니다. 넓은 화면에서는 지역·소방서를 나란히 배치해 세로 길이를 줄입니다.
선택 후 기존 대응 화면으로 이동하며, 지역 선택 자체가 역할 인증이나 전화 연결을 수행하지는 않습니다.
전화 버튼은 단말의 전화 앱을 여는 진입 기능이며, 실제 통화와 브라우저 사고의 자동 연결을 뜻하지 않습니다.
실제 긴급 신고는 **119**로 안내하고, 전화 연동 검증과 records 연동이 진행 중임을 구분합니다.
변경 범위와 후속 개발 기준은 [전화 중심 메인페이지](./docs/PHONE_FIRST_HOME.md)를 참고하세요.
...
- `VITE_ENABLE_SPEECH_API`: 인증된 BE→private Speech API 배포가 검증된 환경에서만 `true`
- `VITE_SPEECH_TIMEOUT_MS`: BE의 Speech timeout 45초보다 긴 client timeout. 기본 50초
...
운영 배경지도는 국내 도로·건물·한글 지명을 제공하는 네이버 Web Dynamic Map을 우선 사용합니다. `VITE_NAVER_MAP_CLIENT_ID`는 등록 도메인으로 제한된 브라우저용 Client ID만 사용하며 Client Secret은 FE에 저장하지 않습니다. 사고·대원 위치와 BE GeoJSON 경로는 네이버 지도 오버레이로 표시합니다. 네이버 설정이 없는 환경에서는 MapTiler Streets Raster XYZ를 MapLibre GL JS로 렌더링하는 fallback을 유지합니다.
...
| `POST /api/c2guard/v1/transcriptions` | 신고 접수 전 음성 전사 client·검토 UX 구현; 배포 검증 대기 |
...
| `POST /api/c2guard/v1/incidents/{incidentId}/transcriptions` | 사고 생성 후 음성 전사 client·검토 UX 구현; 배포 검증 대기 |
